"use client";

import { cn } from "@/lib/utils"
import { ChatBubbleIcon, CodeIcon, DashboardIcon, GearIcon, IconJarLogoIcon, ImageIcon, SpeakerLoudIcon, VideoIcon } from "@radix-ui/react-icons"
import { Montserrat } from "next/font/google"
import Link from "next/link"
import { usePathname } from "next/navigation"

const montserrat = Montserrat({
  weight: '600',
  subsets: ['latin']
})

const routes = [
  {
    label: 'Dashboard',
    icon: DashboardIcon,
    href: '/dashboard',
    color: 'text-sky-500'
  },
  {
    label: 'Coversation',
    icon: ChatBubbleIcon,
    href: '/coversation',
    color: 'text-violet-500'
  },
  {
    label: 'Image Generation',
    icon: ImageIcon,
    href: '/image',
    color: 'text-green-500'
  },
  {
    label: 'Video Generation',
    icon: VideoIcon,
    href: '/video',
    color: 'text-orange-500'
  },
  {
    label: 'Music Generation',
    icon: SpeakerLoudIcon,
    href: '/music',
    color: 'text-teal-500'
  },
  {
    label: 'Code Generation',
    icon: CodeIcon,
    href: '/code',
    color: 'text-indigo-500'
  },
  {
    label: 'Settings',
    icon: GearIcon,
    href: '/settings',
  },
]


const Sidebar = () => {
  const pathname = usePathname()
  
  return (
    <div className=" space-y-4 py-4 flex flex-col h-full bg-slate-800 text-white">
      <div className="px-3 py-2 flex-1">
        <Link href={'/dashboard'} className=" flex items-center pl-3 mb-14">
          {/* <div className=" relative w-8 h-8 mr-4"> */}
          <IconJarLogoIcon className="w-8 h-8 mr-4" />
          {/* </div> */}
          <h1 className={cn("text-2xl font-bold", montserrat.className)}>Treebear AI</h1>
        </Link>
        <div className=" space-y-1">
          {
            routes.map((route) => (
              <Link href={route.href} key={route.href} className={cn("text-sm group flex p-3 w-full justify-start font-medium cursor-pointer hover:text-white hover:bg-white/10 rounded-lg transition",
                pathname === route.href ? "text-white bg-white/10" : "text-zinc-400")}>
                <div className=" flex items-center flex-1">
                  <route.icon className={cn('h-5 w-5 mr-3', route.color)} />
                  {route.label}
                </div>
              </Link>
            ))
          }
        </div>
      </div>
    </div>
  )
}
export default Sidebar
